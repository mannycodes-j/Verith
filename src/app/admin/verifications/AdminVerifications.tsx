...
const statuses = ["DRAFT", "QUEUED", "PROCESSING", "PARTIALLY_COMPLETED", "COMPLETED", "FAILED", "CANCEL_REQUESTED", "CANCELLED", "DELETED"] as const;
...
export default function AdminVerifications() {
	...
	const retry = useMutation({
		mutationFn: () => adminService.retryVerification(retryRecord!.id, reason.trim()),
		onSuccess: () => {
	...
	const invalidUserId = draftUserId.trim().length > 0 && !objectIdPattern.test(draftUserId.trim());
	...
	return (
		<div className={styles.page}>
			<header className={styles.listHero}>
				...
				<p>The administrative projection excludes submitted content. Retry is available only for failed or cancelled records and requires an audit reason.</p>
			</header>
			...
			>
				...
				<label>
					Exact user ID
					<input aria-invalid={invalidUserId} onChange={(event) => setDraftUserId(event.target.value)} placeholder="24-character MongoDB identifier" value={draftUserId} />
							...
							{records.data.items.map((record) => (
								<tr key={record.id}>
									...
									<td>
										...
										<small>{Number.isFinite(record.progress) ? `${record.progress}% reported` : "Progress unavailable"}</small>
									</td>
									<td>
			...
			{records.data && (cursors.length > 0 || records.data.pagination.hasNextPage) && (
				<nav className={styles.pagination} aria-label="Verification result pages">
					<button disabled={cursors.length === 0} onClick={() => setCursors((current) => current.slice(0, -1))} type="button">
						Previous
					</button>
					<span>Cursor page {cursors.length + 1}</span>
					<button disabled={!records.data.pagination.nextCursor} onClick={() => setCursors((current) => [...current, records.data!.pagination.nextCursor!])} type="button">
						Next
					</button>
				</nav>
			)}
			...
			{retryRecord && (
				<div className={styles.dialogBackdrop}>
					...
					>
						...
						<h2 id="retry-title">Requeue verification {retryRecord.id}?</h2>
						<dl>
							<div>
						...
						<p>Verith will enqueue a new processing attempt. The existing record remains the source of truth and the intervention is audit logged.</p>
						<label>
							Operational reason
							<textarea autoFocus minLength={10} onChange={(event) => setReason(event.target.value)} placeholder="Explain why retrying is appropriate" value={reason} />
						</label>
						...
						<footer>
							...
							<button disabled={retry.isPending || reason.trim().length < 10} onClick={() => retry.mutate()} type="button">
