...
export const verifyStyles = {
	page: ui.page,
	header: `${ui.pageHeader} [&>div]:flex [&>div]:flex-wrap [&>div]:gap-3 [&>div_span]:text-[10px] [&>div_span]:font-semibold [&>div_span]:uppercase [&>div_span]:tracking-[0.14em] [&>div_span]:text-violet-400 [&>div_span:first-child]:before:mr-2 [&>div_span:first-child]:before:inline-flex [&>div_span:first-child]:before:size-1.5 [&>div_span:first-child]:before:rounded-full [&>div_span:first-child]:before:bg-violet-400`,
	workspace: "grid grid-cols-[minmax(0,1fr)_360px] items-start gap-6 max-lg:grid-cols-1",
	composer: `${ui.card} min-w-0`,
	tabs: "flex gap-2 overflow-x-auto border-b border-white/[0.05] p-4 [&_button]:rounded-full [&_button]:border [&_button]:border-white/[0.05] [&_button]:bg-white/[0.02] [&_button]:px-5 [&_button]:py-2.5 [&_button]:text-xs [&_button]:font-medium [&_button]:text-white/45 [&_button]:transition-all [&_button:hover]:bg-white/[0.05]",
	activeTab: "!border-violet-400/25 !bg-violet-500/10 !text-violet-300 shadow-[0_0_30px_rgba(139,92,246,0.08)]",
	primaryInput: `grid gap-3 p-1 [&>label]:text-[10px] [&>label]:font-semibold [&>label]:uppercase [&>label]:tracking-[0.14em] [&>label]:text-white/35 [&_textarea]:min-h-64 [&_textarea]:resize-y [&>input]:min-h-14 ${ui.field}`,
