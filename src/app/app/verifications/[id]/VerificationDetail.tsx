...
import { verificationService, type VerificationRecord } from "@/services/verification";
...
export default function VerificationDetail({ id }: { id: string }) {
	...
	useEffect(() => {
		...
		const existingEvents = queryClient.getQueryData<Awaited<ReturnType<typeof verificationService.listEvents>>>(["verification-events", id]) ?? [];
		...
		void verificationService
			.streamEvents({
				...
				id,
				onEvent: (event) => {
					queryClient.setQueryData(["verification-events", id], (current: Awaited<ReturnType<typeof verificationService.listEvents>> | undefined) => {
						const events = current ?? [];
						return events.some((item) => item.sequence === event.sequence) ? events : [...events, event].sort((a, b) => a.sequence - b.sequence);
					});
					queryClient.setQueryData(["verification", id], (current: VerificationRecord | undefined) =>
						current
							? {
									...current,
		...
		<div className={styles.page}>
			<header className={styles.header}>
				...
				<div className={styles.actions}>
					{canCancel && (
						<button data-variant="danger" type="button" disabled={cancel.isPending} onClick={() => setActionDialog("cancel")}>
					...
					{canDelete && (
						<button data-variant="danger" type="button" onClick={() => setActionDialog("delete")}>
							Delete
						</button>
					)}
			...
			{actionDialog && (
				<div aria-labelledby="verification-action-title" aria-modal="true" className={styles.dialogBackdrop} role="dialog">
					<section className={styles.dialog}>
						<header>
							<span>Case: {record.id}</span>
							<button aria-label="Close dialog" disabled={actionPending} onClick={() => setActionDialog(null)} type="button">
								×
							</button>
						</header>
						...
						<footer>
							<button disabled={actionPending} onClick={() => setActionDialog(null)} type="button">
								Keep case
							</button>
							<button
