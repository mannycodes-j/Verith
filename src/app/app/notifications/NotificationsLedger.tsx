...
export default function NotificationsLedger() {
	...
	const unread = unreadStatus.data?.unreadCount ?? records.filter((record) => !record.readAt).length;
	...
	return (
		<div className={styles.page}>
			<header className={styles.header}>
				...
				<div>
					<p>Follow investigation milestones, evidence updates, learning progress, and security events from one focused activity stream.</p>
					<button disabled={readAll.isPending || unread === 0} onClick={() => readAll.mutate()} type="button">
			...
			{!notifications.isPending && !notifications.isError && records.length === 0 && (
				<section className={styles.empty}>
					<span>No notifications</span>
					<h2>Your activity ledger is empty.</h2>
					<p>Verith does not generate demonstration notifications. Real investigation and account events will appear here.</p>
				</section>
			)}
			{records.length > 0 && (
					...
					{records.map((record) => (
						<li data-read={Boolean(record.readAt)} key={record._id}>
							...
							<div>
								...
								<p>{record.message}</p>
								<small>
									{formatDate(record.createdAt)} · Email {record.emailStatus.replaceAll("_", " ").toLowerCase()}
								</small>
							</div>
							<div className={styles.recordActions}>
								{record.actionUrl?.startsWith("/") && <Link href={record.actionUrl}>Open</Link>}
								{!record.readAt && (
									<button disabled={read.isPending} onClick={() => read.mutate(record._id)} type="button">
										Mark read
									</button>
								)}
								{/* <button
			...
			{notifications.hasNextPage && (
				<button className={styles.loadMore} disabled={notifications.isFetchingNextPage} onClick={() => void notifications.fetchNextPage()} type="button">
					{notifications.isFetchingNextPage ? "Loading more…" : "Load older notifications"}
				</button>
			)}
		</div>
	);
