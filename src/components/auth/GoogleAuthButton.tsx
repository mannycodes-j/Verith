...
const googleErrors: Record<string, string> = {
  ACCOUNT_NOT_ACTIVE:
    "This account is currently unavailable. Contact support for assistance.",
  GOOGLE_ACCOUNT_ALREADY_EXISTS:
    "This Google account already has a Verith workspace. Use Google on the login page.",
  GOOGLE_ACCOUNT_NOT_FOUND:
    "No Google-created Verith account was found. Create one from the registration page first.",
  GOOGLE_CREDENTIAL_INVALID:
    "Google could not verify that sign-in response. Please try again.",
  GOOGLE_EMAIL_ALREADY_REGISTERED:
    "That email already belongs to a password account. Sign in with email and password.",
  GOOGLE_IDENTITY_INCOMPLETE:
    "Google did not provide a verified email identity for this account.",
  USE_PASSWORD_SIGN_IN:
    "This account was created with email and password. Use the password login form.",
};
...
export default function GoogleAuthButton({
      ...
      try {
        ...
        setError(
          reason instanceof ApiClientError
            ? (googleErrors[reason.code] ?? reason.message)
            : "Verith could not complete Google authentication.",
        );
      } finally {
        setPending(false);
      }
  ...
  useEffect(() => {
    ...
    google.renderButton(target, {
      ...
      size: "large",
      logo_alignment: "left",
      width: Math.max(240, Math.min(400, target.clientWidth || 400)),
    });
  }, [authenticate, configuration.data?.clientId, intent, scriptReady]);

  const unavailable =
    configuration.isError ||
    (configuration.data && !configuration.data.enabled);
    ...
    <section className={styles.providerAuth} aria-busy={pending}>
      ...
      {(configuration.isPending || pending) && (
        <p className={styles.providerStatus} role="status">
          {pending
            ? "Establishing your secure Google session…"
