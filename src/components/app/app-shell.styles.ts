export const appShellStyles = {
  shell:
    "grid min-h-svh grid-cols-[260px_minmax(0,1fr)] bg-background max-[1120px]:grid-cols-[76px_minmax(0,1fr)] max-[767px]:block",
  sidebar:
    "sticky top-0 flex h-svh flex-col border-r border-white/[0.07] bg-[#050505]/95 backdrop-blur-xl max-[767px]:hidden",
  sidebarHeader:
    "grid min-h-20 gap-1 border-b border-white/[0.07] px-5 py-5 max-[1120px]:place-items-center max-[1120px]:px-2 [&>span]:font-mono [&>span]:text-[0.57rem] [&>span]:tracking-[0.14em] [&>span]:text-muted [&>span]:uppercase max-[1120px]:[&>span]:text-[0px]",
  wordmark:
    "flex items-center gap-2.5 text-[1.05rem] font-semibold tracking-[-0.03em] [&>span]:flex [&>span]:size-8 [&>span]:shrink-0 [&>span]:items-center [&>span]:justify-center [&>span]:rounded-lg [&>span]:bg-accent [&>span]:text-sm [&>span]:font-bold [&>span]:text-accent-foreground [&>span]:shadow-[0_0_20px_-8px_rgba(245,158,11,0.8)] max-[1120px]:[&>strong]:hidden",
  navigation:
    "flex flex-1 flex-col gap-8 overflow-y-auto px-3.5 py-6 max-[1120px]:px-2.5 max-[1120px]:py-5",
  navGroup: "grid gap-1",
  navLabel:
    "px-2.5 pb-2 font-mono text-[0.57rem] tracking-[0.16em] text-muted uppercase max-[1120px]:text-[0px] max-[767px]:text-[0.72rem]",
  navItem:
    "grid min-h-10 grid-cols-[1.6rem_1fr] items-center gap-3 rounded-lg border border-transparent px-2.5 text-[0.76rem] text-muted-foreground transition-all duration-200 hover:border-white/[0.08] hover:bg-white/[0.035] hover:text-foreground max-[1120px]:flex max-[1120px]:justify-center max-[1120px]:px-0 max-[767px]:grid max-[767px]:justify-normal max-[767px]:px-2.5 max-[767px]:text-[0.72rem] [&>span]:font-mono [&>span]:text-[0.55rem] [&>span]:text-muted max-[1120px]:[&>span]:text-[0.6rem]",
  navItemActive:
    "border-amber-400/15 bg-amber-400/[0.08] text-foreground shadow-[inset_2px_0_0_#ffc438] [&>span]:text-accent",
  account:
  ...
  avatar:
    "flex size-8.5 items-center justify-center rounded-lg border border-white/10 bg-white/[0.04] text-xs font-bold",
  workspace: "min-w-0",
  topbar:
    "sticky top-0 z-30 flex h-20 items-center justify-between border-b border-white/[0.07] bg-black/75 px-[clamp(1rem,3vw,2.5rem)] backdrop-blur-xl max-[767px]:h-16 [&>div:first-child]:grid [&>div:first-child]:gap-1.5 [&>div:first-child_span]:font-mono [&>div:first-child_span]:text-[0.57rem] [&>div:first-child_span]:tracking-[0.14em] [&>div:first-child_span]:text-muted [&>div:first-child_span]:uppercase max-[767px]:[&>div:first-child_span]:hidden [&_strong]:text-sm [&_strong]:font-medium",
  topbarActions:
    "flex items-center gap-2.5 [&>a]:min-h-9.5 [&>a]:rounded-full [&>a]:border [&>a]:border-white/10 [&>a]:bg-white/[0.03] [&>a]:px-4 [&>a]:py-3 [&>a]:font-mono [&>a]:text-[0.58rem] [&>a]:tracking-[0.06em] [&>a]:uppercase [&>a]:transition-all [&>a:hover]:border-white/20 [&>a:hover]:bg-white/[0.06] [&>a:last-of-type]:border-amber-300/30 [&>a:last-of-type]:bg-gradient-to-r [&>a:last-of-type]:from-[#ffebb1] [&>a:last-of-type]:to-[#ffc438] [&>a:last-of-type]:text-[#18130a] max-[767px]:[&>a:first-child]:hidden max-[767px]:[&>a:last-of-type]:min-h-9 max-[767px]:[&>a:last-of-type]:p-3 max-[430px]:[&>a:last-of-type]:hidden",
  notificationTrigger:
    "min-h-9.5 rounded-full border border-white/10 bg-white/[0.03] px-3.5 py-3 font-mono text-[0.58rem] tracking-[0.06em] uppercase transition-colors hover:bg-white/[0.07]",
  mobileNavigation:
  ...
  content:
    "min-h-[calc(100svh-80px)] p-[clamp(1.25rem,3vw,2.5rem)] max-[767px]:min-h-[calc(100svh-64px)] max-[767px]:p-4",
  loadingGate:
  ...
  loadingMark:
    "flex size-11 items-center justify-center rounded-xl border border-amber-400/20 bg-amber-400/[0.08] font-bold text-accent shadow-[0_0_28px_-12px_rgba(245,158,11,0.8)]",
  gate:
