...
export default function PublicNavbar({
  ...
  return (
    <>
      <a
        className="fixed top-4 left-4 z-[70] -translate-y-[160%] rounded-full bg-white px-4 py-3 text-sm font-medium text-black transition-transform focus:translate-y-0"
        href={`#${mainId}`}
      >
        Skip to content
      </a>
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <Link className="hidden items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-medium text-black transition-transform hover:scale-105 active:scale-95 md:flex shadow-[0_0_15px_rgba(255,255,255,0.15)]" href="/login">
              ...
              >
                <div className="grid gap-1">
                  {publicLinks.map((link, index) => (
                    ...
                    >
                      <Link className="flex items-center justify-between rounded-xl px-4 py-3 text-sm font-medium text-white/70 transition hover:bg-white/[.04] hover:text-white" href={link.href} onClick={() => setMenuOpen(false)}>
                        {link.label}<ArrowRight aria-hidden="true" size={14} className="opacity-50" />
                      </Link>
                    </motion.div>
                  ))}
                  <div className="mt-2 border-t border-white/[.06] pt-3">
                    <Link className="flex w-full items-center justify-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-medium text-black transition active:scale-95" href="/login" onClick={() => setMenuOpen(false)}>
